...
\begin{figure}[!ht]
    \begin{center}
        \resizebox{\textwidth}{!}{\input{5/figures/fig1_prompt_absa}}
...
\begin{figure}[!ht]
    \begin{center}
        \resizebox{\textwidth}{!}{\input{5/figures/fig2_arquitectura_temas}}
        \caption[Arquitectura del módulo de detección dinámica de temas con LLM y retrieval multinivel]{Arquitectura del módulo de detección dinámica de temas electorales: \textit{embeddings\\Fuente: Elaboración propia.}
        \label{6:fig2}
    \end{center}
\end{figure} multilingües, menú de cinco \textit{tiers} (\textit{trending}, semántico, léxico, actor, evento), llamada concurrente al LLM con \textit{prompt} de extracción, \textit{override} semántico post-LLM y \textit{dedup} intra-\textit{batch} previo al \textit{upsert} en \texttt{temas\_conocidos}.}
...
\begin{figure}[!ht]
    \begin{center}
        \resizebox{\textwidth}{!}{\input{5/figures/fig3_grafo_neo4j_schema}}
        \caption[Esquema final del grafo de conocimiento electoral implementado en Neo4j]{Esquema final del grafo de conocimiento electoral en Neo4j Community, con sus diez tipos de nodos (\texttt{Documento\\Fuente: Elaboración propia.}
        \label{6:fig3}
    \end{center}
\end{figure}, \texttt{Fragmento}, \texttt{Tema}, \texttt{Actor}, \texttt{Candidato}, \texttt{Categoria}, \texttt{Encuesta}, \texttt{TipoCambio}, \texttt{IndicadorMacro}, \texttt{SentimientoDiario}) y siete tipos de relaciones (\texttt{CONTIENE}, \texttt{SOBRE\_TEMA}, \texttt{MENCIONA}, \texttt{MENCIONA\_ACTOR}, \texttt{DE\_CATEGORIA}, \texttt{MIDE}, \texttt{TIENE\_SENTIMIENTO\_EN}) con su respectiva metadata de aristas.}
...
archivo \texttt{docs/pendientes-evaluacion.md} del repositorio
\texttt{electoral-intelligence}.
...
queda como trabajo futuro y se dimensiona en
...
procesamiento, y se documentan como trabajo futuro en
\texttt{docs/pendientes-evaluacion.md}, sección 2.
...
\begin{figure}[!ht]
    \begin{center}
        \resizebox{\textwidth}{!}{\input{5/figures/fig9_ejemplos_mcp}}
...
(\texttt{docs/pendientes-evaluacion.md}, sección 3).
...
(ver \texttt{docs/pendientes-evaluacion.md}, sección 4).

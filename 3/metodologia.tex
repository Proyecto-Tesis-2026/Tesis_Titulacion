...
& Construcción del corpus de publicaciones y metadatos de videos cortos en TikTok.
& \begin{itemize}[label={--},nosep,leftmargin=*]
\item Recolectar publicaciones mediante scraping de cuentas y hashtags.
\item Extraer descripciones, hashtags y métricas de viralidad (likes, compartidos).
\item Almacenar los datos crudos en formato JSON en la capa Bronze.
\end{itemize} \\
\hline
...
La recolección de contenido de la plataforma TikTok se llevó a cabo mediante un módulo de \textit{scraping} especializado ubicado en \texttt{scrap-information/tiktok}, dado que la plataforma impone restricciones significativas y no cuenta con una API pública orientada a investigación. El proceso se automatizó para extraer publicaciones a partir de un catálogo de cuentas oficiales de los candidatos y una lista de \textit{hashtags} de campaña. De cada video capturado se extrajo la descripción (\textit{caption}), la lista de etiquetas (\textit{hashtags}), el identificador del autor, la fecha de publicación y las métricas de viralidad (reproducciones, \textit{likes}, comentarios, compartidos y guardados). Los resultados se guardan en formato JSON en la capa Bronze.

% \begin{figure}[!ht]
%     \begin{center}
%         \resizebox{0.85\textwidth}{!}{\input{3/figures/flujo_tiktok.tex}}
%         \caption[Proceso de recolección del corpus de TikTok]{Proceso de recolección del corpus de TikTok: configuración de reglas de cuentas/hashtags, extracción de metadatos de videos cortos y almacenamiento en capa Bronze.\\
%             Fuente: Elaboración propia.}
%         \label{4:fig_tiktok}
%     \end{center}
% \end{figure}
 
\textbf{Entregable}: Corpus de publicaciones de TikTok con ID, descripción, autor, fecha y métricas de interacción.

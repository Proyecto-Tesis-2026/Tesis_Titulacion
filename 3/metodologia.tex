...
\begin{longtable}{M{3cm}M{4.2cm}M{3.6cm}M{4.8cm}}
    ...
    \multirow{10}{3cm}[-7ex]{\centering Dependiente: Percepción pública nacional en el ecosistema digital}
    & \multirow{3}{4.2cm}[-2ex]{\centering Sentimiento ciudadano por Actor y Evento}
    & \multirow{2}{3.6cm}[0ex]{\centering Polaridad y Contexto}
    & $Positivo: S(d,e) > 0$ \\
    \cline{4-4}
    & & & $Negativo: S(d,e) < 0$ \\
    \cline{3-4}
    & & Intensidad y Comparación & Índice de Sentimiento Neto (ISN) y Evolución de sentimiento $[-1, 1]$ \\
    \cline{2-4}
    & \multirow{5}{4.2cm}[-2ex]{\centering Conversación y Panorama Digital}
    & \multirow{2}{3.6cm}[0ex]{\centering Volumen y Fuentes}
    & Cantidad de publicaciones por actor/período \\
    \cline{4-4}
    & & & Distribución (Noticias, X, TikTok, YouTube) \\
    \cline{3-4}
    & & Panorama temático & Índice de Relevancia Temática (IRT) y Top Temas por período \\
    \cline{3-4}
    & & \multirow{2}{3.6cm}[0ex]{\centering Evidencia Causal}
    & Densidad de enlaces CITA/REACCIONA\_A en el grafo \\
...
El estudio tiene como objetivo abordar el problema del análisis de la percepción pública a nivel nacional en ecosistemas digitales de carácter electoral. Para ello, implementa un pipeline automatizado de inteligencia electoral que integra análisis de sentimientos, deduplicación de entidades y extracción semántica utilizando Modelos Fundacionales (LLMs), almacenando los hallazgos en una arquitectura de base de datos dual: vectorial (\textit{pgvector}) para similitud semántica y de grafos (\textit{Neo4j}) para la interrelación de actores, temas y eventos.

La variable dependiente —la percepción pública nacional— se operacionaliza a través del sentimiento expresado en las publicaciones digitales hacia cada candidato o actor político, desagregado por fuente de información, período temporal y tema asociado. Siguiendo un enfoque \textit{Aspect-Based Sentiment Analysis} enriquecido por la topología del grafo, el sistema permite que herramientas expuestas vía el Model Context Protocol (MCP) construyan de manera dinámica panoramas diarios, comparen sentimientos entre candidatos y tracen la evolución de la percepción pública con una alta trazabilidad documental (Evidencia RAG).
...
\begin{itemize}
    ...
    \item \textbf{Módulo de YouTube (\textit{API \& Scraping}):} Encargado de la extracción de metadatos de videos electorales, transcripciones de contenido audiovisual y comentarios de los ciudadanos, separando el discurso oficial del candidato de la reacción del público.
...
Durante la ejecución de la metodología, se definió construir un \textit{pipeline} analítico avanzado que integra análisis de sentimientos basado en aspectos (ABSA) orquestado por múltiples Modelos Fundacionales (LLMs), generación de \textit{embeddings} semánticos, almacenamiento vectorial y un grafo de conocimiento. A esta arquitectura dual se le denomina \textbf{GraphRAG Híbrido Electoral}. De acuerdo a la literatura de los antecedentes revisados,
...
\begin{longtable}{m{4.6cm}m{4.9cm}M{5.5cm}}
    \caption[Plan de Ejecución de la fase Construcción del modelo]{Plan de Ejecución de la fase Construcción del modelo (GraphRAG Híbrido).}
    \label{4:table6}
    \footnotesize
    \small
    \tabularnewline \specialrule{.1em}{.05em}{.05em}
    \centering Plan de Ejecución & \centering Alcance & Acciones \\
    \specialrule{.1em}{.05em}{.05em}
    Extracción Estructurada con LLM Orquestado.
    & Análisis de sentimiento a nivel de aspecto (ABSA) y extracción de entidades mediante Modelos Fundacionales.
    & \setlist{nolistsep}
    \begin{itemize}[label={--},nosep,noitemsep,leftmargin=*,topsep=0pt,partopsep=0pt]
        \item Utilizar DeepSeek-V4 para \textit{parsing} rápido y estructuración JSON.
        \item Extraer actores, temas, eventos, polaridad y fragmentos de evidencia textual.
    \end{itemize}
    \\
    \hline
    Deduplicación y Resolución Semántica (LLM Juez).
    & Consolidación ontológica de entidades emergentes (actores y temas) para evitar la fragmentación del grafo.
    & \setlist{nolistsep}
    \begin{itemize}[label={--},nosep,noitemsep,leftmargin=*,topsep=0pt,partopsep=0pt]
        \item Ejecutar proceso de deduplicación intra-lote y cruce con catálogos YAML.
        \item Utilizar modelos de alto razonamiento (ej. DeepSeek) como "LLM Juez" para fusionar entidades similares.
    \end{itemize}
    \\
    \hline
    Construcción del Grafo de Conocimiento (Capa Gold).
    & Implementación topológica en Neo4j interconectando el ecosistema digital.
    & \setlist{nolistsep}
    \begin{itemize}[label={--},nosep,noitemsep,leftmargin=*,topsep=0pt,partopsep=0pt]
        \item Ingestar nodos (Actor, Tema, Evento, Fuente, Encuesta).
        \item Trazar relaciones temporales y causales (\texttt{MENCIONA}, \texttt{ASOCIADO\_A}, \texttt{EVOLUCIONA}).
    \end{itemize}
    \\
    \hline
    Desarrollo del Servidor Model Context Protocol (MCP).
    & Creación de una capa de intermediación estándar para permitir a agentes IA interactuar con la base de datos dual.
    & \setlist{nolistsep}
    \begin{itemize}[label={--},nosep,noitemsep,leftmargin=*,topsep=0pt,partopsep=0pt]
        \item Desarrollar \textit{endpoints} de búsqueda, panorama diario y evidencia en Python.
        \item Integrar rutinas SQL y Cypher abstraídas como "herramientas" (\textit{tools}).
    \end{itemize}
    \\
    \specialrule{.1em}{.05em}{.05em}
\end{longtable}
